class Libro {
  ...
  constructor(titulo, autor, isbn) {
    if (typeof titulo !== 'string' || (titulo.trim() === '' && typeof autor !== 'string') || autor.trim() === '') {
      throw new Error('El título o el autor no pueden ser cadenas vacias o cadenas en blanco')
    }
    this.#autor = autor
...
class Biblioteca {
  ...
  listarLibrosDisponibles() {
    return Array.from(this.#catalogo.values()).filter((libro) => libro.disponible)
  }
    ...
    if (this.#catalogo.has(isbn)) {
      ...
      console.log(
        `El libro ${libroEncontrado.titulo} ${
          libroEncontrado.disponible ? 'se encuentra disponible' : 'no se encuentra disponible en este momento'
        }`,
      )
    }
  }
...
try {
  ...
  const libro3 = miBiblioteca.agregarLibro('Hiliada', 'Homero', '978-0451524578')

  const libro4 = miBiblioteca.agregarLibro('Hitch', 'J.P Morgan', '978-0451524114')
} catch (error) {
